...
{
    /// <inheritdoc />
    public partial class funcionScalar : Migration
    {
        /// <inheritdoc />
...
BEGIN
	...
	if @DifMes =1
		...
		if @MesFin=2 and @DiaFin=28
			...
			SET  @DiasGenerados = ((30-(@DiaIni -2))*@TASA  + (@DiaFin*@TASA))
		END
	...
	if @DifMes > 1
		...
		if @MesFin=2 and @DiaFin=28
			BEGIN
				...
				ELSE
					--SET  @DiasGenerados = ((30-(@DiaIni -2))*@TASA  + (30*@TASA))
					SET @DiasGenerados = (30-(@DiaIni -2))*@TASA + ((@DifMes-1)*2.5)  + (30*@TASA)
			END
			...
			set @DiasGenerados = (30-(@DiaIni -2))*@TASA + ((@DifMes-1)*2.5)  + (@DiaFin*@TASA)
		END
